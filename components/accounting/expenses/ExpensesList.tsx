        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            {totalCount !== undefined && (
              <p className="text-[11px] text-secondary/90 font-dubai">{new Intl.NumberFormat(locale).format(totalCount!)} {t.accounting.common.expense}</p>
            )}
          </div>
        </div>
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16">
          <div className="w-12 h-12 rounded-2xl bg-secondary/[0.03] mx-auto mb-3 flex items-center justify-center">
            <Receipt size={22} className="text-secondary/80" />
          </div>
          <p className="text-secondary/90 font-dubai text-sm">{t.accounting.expenses.noExpensesFilter}</p>
        </div>
      ) : (
        <>
          {/* Mobile Card View */}
              ...
              >
                ...
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[11px] text-secondary/90 font-dubai">{formatDate(expense.date)}</span>
                  </div>
            ...
            <table className="w-full text-sm">
              <thead className="sticky top-0 z-10">
                <tr className="bg-secondary/[0.02] border-b border-secondary/[0.06]">
                  {showApartment && (
                    <th className="px-4 py-3 text-right text-[11px] text-secondary/90 font-medium font-dubai"><span className="inline-flex items-center gap-1"><Building2 size={12} />{t.accounting.expenses.apartment}</span></th>
                  )}
                  <th className="px-4 py-3 text-right text-[11px] text-secondary/90 font-medium font-dubai"><span className="inline-flex items-center gap-1"><FileText size={12} />{t.accounting.expenses.description}</span></th>
                  <th className="px-4 py-3 text-center text-[11px] text-secondary/90 font-medium font-dubai"><span className="inline-flex items-center gap-1"><Tag size={12} />{t.accounting.expenses.category}</span></th>
                  <th className="px-4 py-3 text-center text-[11px] text-secondary/90 font-medium font-dubai hidden sm:table-cell"><span className="inline-flex items-center gap-1"><Calendar size={12} />{t.accounting.expenses.date}</span></th>
                  <th className="px-4 py-3 text-center text-[11px] text-secondary/90 font-medium font-dubai"><span className="inline-flex items-center gap-1"><DollarSign size={12} />{t.accounting.expenses.amount}</span></th>
                  <th className="px-4 py-3 text-center text-[11px] text-secondary/90 font-medium font-dubai"><span className="inline-flex items-center gap-1"><CircleDot size={12} />{t.accounting.expenses.status}</span></th>
                  {hasActions && (
                    <th className="px-4 py-3 text-center text-[11px] text-secondary/90 font-medium font-dubai w-24"><span className="inline-flex items-center gap-1"><Settings size={12} />{t.accounting.expenses.actions}</span></th>
                  )}
                </tr>
              </thead>
              <tbody>
                {expenses.map((expense, i) => (
                  ...
                  >
                    ...
                    <td className="px-4 py-3.5 font-dubai">
                      ...
                      {expense.notes && (
                        <span className="text-[10px] text-secondary/90 flex items-center gap-0.5 mt-0.5" title={expense.notes}>
                          <FileText size={9} />
