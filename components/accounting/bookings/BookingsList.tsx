...
const SOURCE_BADGE_CLASSES: Record<string, string> = {
  ...
  OTHER: 'bg-secondary/5 text-secondary/80 ring-1 ring-secondary/10',
};

const STATUS_BADGE_CLASSES: Record<string, string> = {
  CONFIRMED: 'bg-sky-50 text-sky-600',
  CHECKED_IN: 'bg-emerald-50 text-emerald-600',
  CHECKED_OUT: 'bg-secondary/5 text-secondary/75',
        ...
        <div className="flex items-center gap-2.5">
          ...
          <div>
            ...
            {totalCount !== undefined && (
              <p className="text-[11px] text-secondary/70 font-dubai">{new Intl.NumberFormat(locale).format(totalCount!)} {t.accounting.common.booking}</p>
            )}
          </div>
        </div>
        <div className="flex items-center gap-2">
          {totalNights !== undefined && totalNights > 0 && !isLoading && (
            <span className="text-[11px] font-bold text-secondary/75 font-dubai bg-secondary/[0.04] px-2.5 py-1 rounded-lg">
      ...
      {isLoading ? (
        ...
        <div className="text-center py-16">
          <div className="w-12 h-12 rounded-2xl bg-secondary/[0.03] mx-auto mb-3 flex items-center justify-center">
            <CalendarCheck size={22} className="text-secondary/60" />
          </div>
          <p className="text-secondary/70 font-dubai text-sm">{t.accounting.bookings.noBookingsFilter}</p>
        </div>
      ) : (
        <>
          {/* Mobile Card View */}
            ...
            {bookings.map((booking, i) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="flex flex-wrap items-center gap-2 text-[11px] text-secondary/75 font-dubai">
                    <span>{formatDate(booking.checkIn)} → {formatDate(booking.checkOut)}</span>
                    <span className="font-bold text-secondary/80">{new Intl.NumberFormat(locale).format(booking.nights)} {t.accounting.common.night}</span>
                  </div>
            ...
            <table className="w-full text-sm">
              <thead className="sticky top-0 z-10">
                <tr className="bg-secondary/[0.02] border-b border-secondary/[0.06]">
                  {showApartment && <th className="px-4 py-3 text-right text-[11px] text-secondary/70 font-medium font-dubai"><span className="inline-flex items-center gap-1"><Building2 size={12} />{t.accounting.bookings.apartment}</span></th>}
                  <th className="px-4 py-3 text-right text-[11px] text-secondary/70 font-medium font-dubai"><span className="inline-flex items-center gap-1"><User size={12} />{t.accounting.bookings.client}</span></th>
                  <th className="px-4 py-3 text-center text-[11px] text-secondary/70 font-medium font-dubai hidden sm:table-cell"><span className="inline-flex items-center gap-1"><LogIn size={12} />{t.accounting.bookings.checkIn}</span></th>
                  <th className="px-4 py-3 text-center text-[11px] text-secondary/70 font-medium font-dubai hidden sm:table-cell"><span className="inline-flex items-center gap-1"><LogOut size={12} />{t.accounting.bookings.checkOut}</span></th>
                  <th className="px-4 py-3 text-center text-[11px] text-secondary/70 font-medium font-dubai"><span className="inline-flex items-center gap-1"><Moon size={12} />{t.accounting.bookings.nights}</span></th>
                  {!hideFinancials && <th className="px-4 py-3 text-center text-[11px] text-secondary/70 font-medium font-dubai"><span className="inline-flex items-center gap-1"><DollarSign size={12} />{t.accounting.bookings.amount}</span></th>}
                  <th className="px-4 py-3 text-center text-[11px] text-secondary/70 font-medium font-dubai"><span className="inline-flex items-center gap-1"><Globe size={12} />{t.accounting.bookings.source}</span></th>
                  <th className="px-4 py-3 text-center text-[11px] text-secondary/70 font-medium font-dubai hidden md:table-cell"><span className="inline-flex items-center gap-1"><CircleDot size={12} />{t.accounting.bookings.status}</span></th>
                  {hasActions && <th className="px-4 py-3 text-center text-[11px] text-secondary/70 font-medium font-dubai w-20"><span className="inline-flex items-center gap-1"><Settings size={12} />{t.accounting.bookings.actions}</span></th>}
                </tr>
              </thead>
              <tbody>
                {bookings.map((booking, i) => {
                    ...
                    >
                      ...
                      <td className="px-4 py-3.5 font-dubai">
                        ...
                        <div className="flex items-center gap-2 mt-0.5">
                          {booking.clientPhone && (
                            <span className="text-[10px] text-secondary/80 flex items-center gap-0.5 ltr">
                              <Phone size={9} />
                              {booking.clientPhone}
                            </span>
                          )}
                          {booking.arrivalTime && (
                            <span className="text-[10px] text-secondary/80 flex items-center gap-0.5">
                              <Clock size={9} />
                              {booking.arrivalTime}
                            </span>
                          )}
                          {booking.notes && (
                            <span className="text-[10px] text-secondary/70" title={booking.notes}>
                              <FileText size={9} />
                            </span>
                          )}
                        </div>
