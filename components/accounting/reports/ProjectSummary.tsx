...
const ProjectSummary: React.FC<ProjectSummaryProps> = ({
  ...
  return (
    <div className="space-y-5">
      ...
      >
        ...
        <div className="flex items-center gap-2.5 px-5 pt-5 pb-3">
          ...
          <div>
            ...
            <p className="text-[11px] text-white/90 font-dubai">{t.accounting.reports.allApartmentsTotal}</p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-2.5 sm:gap-3 px-4 sm:px-5 pb-5 pt-2">
          ...
          >
            ...
            <p className="text-[11px] sm:text-xs text-white/90 font-dubai mb-1">{t.accounting.dashboard.revenue}</p>
          ...
          >
            ...
            <p className="text-[11px] sm:text-xs text-white/90 font-dubai mb-1">{t.accounting.dashboard.expensesLabel}</p>
          ...
          >
            ...
            <p className="text-[11px] sm:text-xs text-white/90 font-dubai mb-1">{t.accounting.dashboard.netProfit}</p>
          ...
          >
            ...
            <p className="text-[11px] sm:text-xs text-white/90 font-dubai mb-1">{t.accounting.reports.apartments}</p>
      ...
      {/* Per-apartment cards */}
      {apartments.length === 0 ? (
        <div className="text-center py-10 text-secondary/90 font-dubai text-sm">
              ...
              >
                <div className="flex items-center justify-between mb-3">
                  <div className="flex items-center gap-2">
                    ...
                    <div>
                      ...
                      {apt.project && (
                        <p className="text-[10px] text-secondary/90 font-dubai">{apt.project}</p>
                      )}
                    </div>
                  </div>
                  <div className={`text-[10px] font-bold px-2 py-0.5 rounded-full font-dubai
                    ${apt.profit >= 0 ? 'bg-primary/15 text-secondary' : 'bg-secondary/10 text-secondary/90'}`}
                  >
                ...
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-xs font-dubai">
                    <span className="text-secondary/90">{t.accounting.dashboard.revenue}</span>
                    <span className="font-bold text-secondary">{formatCurrency(apt.revenue)}</span>
                  </div>
                  <div className="flex items-center justify-between text-xs font-dubai">
                    <span className="text-secondary/90">{t.accounting.dashboard.expensesLabel}</span>
                    <span className="font-bold text-secondary/90">{formatCurrency(apt.expenses)}</span>
                  </div>
                  <div className="h-px bg-secondary/[0.06]" />
                  <div className="flex items-center justify-between text-xs font-dubai">
                    <span className="text-secondary/90 font-bold">{t.accounting.dashboard.netProfit}</span>
                    <span className={`font-bold ${apt.profit >= 0 ? 'text-secondary' : 'text-secondary/90'}`}>
                ...
                <div className="flex items-center gap-3 mt-3 pt-3 border-t border-primary/10">
                  <div className="flex items-center gap-1 text-[10px] text-secondary/90 font-dubai">
