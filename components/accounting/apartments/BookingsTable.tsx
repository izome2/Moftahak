...
const STATUS_CLASSES: Record<string, string> = {
  ...
  CHECKED_OUT: 'bg-secondary/10 text-secondary/90',
    ...
    >
      ...
      {isLoading ? (
        ...
        <div className="text-center py-14">
          ...
          <p className="text-secondary/90 font-dubai text-sm">{t.accounting.apartments.noBookingsThisMonth}</p>
        </div>
      ) : (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-secondary/[0.02] border-b border-secondary/[0.06]">
                <th className="px-4 py-3 text-right text-[11px] text-secondary/90 font-medium font-dubai"><span className="inline-flex items-center gap-1"><User size={12} />{t.accounting.apartments.client}</span></th>
                <th className="px-4 py-3 text-center text-[11px] text-secondary/90 font-medium font-dubai hidden sm:table-cell"><span className="inline-flex items-center gap-1"><LogIn size={12} />{t.accounting.apartments.checkInDate}</span></th>
                <th className="px-4 py-3 text-center text-[11px] text-secondary/90 font-medium font-dubai hidden sm:table-cell"><span className="inline-flex items-center gap-1"><LogOut size={12} />{t.accounting.apartments.checkOutDate}</span></th>
                <th className="px-4 py-3 text-center text-[11px] text-secondary/90 font-medium font-dubai"><span className="inline-flex items-center gap-1"><Moon size={12} />{t.accounting.apartments.nights}</span></th>
                {!hideAmounts && <th className="px-4 py-3 text-center text-[11px] text-secondary/90 font-medium font-dubai"><span className="inline-flex items-center gap-1"><DollarSign size={12} />{t.accounting.apartments.amountHeader}</span></th>}
                <th className="px-4 py-3 text-center text-[11px] text-secondary/90 font-medium font-dubai"><span className="inline-flex items-center gap-1"><Globe size={12} />{t.accounting.apartments.source}</span></th>
                <th className="px-4 py-3 text-center text-[11px] text-secondary/90 font-medium font-dubai hidden md:table-cell"><span className="inline-flex items-center gap-1"><CircleDot size={12} />{t.accounting.apartments.status}</span></th>
              </tr>
            </thead>
            <tbody>
                ...
                return (
                  ...
                  >
                    <td className="px-4 py-3.5 font-dubai">
                      ...
                      {booking.clientPhone && (
                        <p className="text-[10px] text-secondary/90 mt-0.5 ltr">{booking.clientPhone}</p>
                      )}
                    </td>
