  ...
  return (
    <div className="space-y-4">
      ...
      {isLoading ? (
        ...
        <p className="text-xs text-secondary/90 font-dubai text-center py-4">{t.accounting.settings.team.noMembers}</p>
      ) : (
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className={`text-[9px] rounded px-1.5 py-0.5 font-bold font-dubai ${ROLE_COLORS[m.role] || 'bg-secondary/10 text-secondary/90'}`}>
                    {ROLE_LABELS[m.role] || m.role}
                  </span>
                  {m.additionalRoles && m.additionalRoles.length > 0 && m.additionalRoles.map(r => (
                    <span key={r} className={`text-[9px] rounded px-1.5 py-0.5 font-bold font-dubai ${ROLE_COLORS[r] || 'bg-secondary/10 text-secondary/90'}`}>
                      +{ROLE_LABELS[r] || r}
                    </span>
                  ))}
                </div>
                <div className="flex items-center gap-3 text-[10px] text-secondary/90 font-dubai mt-0.5">
                  {m.email && (
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            >
              <div className="flex items-center justify-between px-5 py-4 border-b border-secondary/[0.06]">
                ...
                <button onClick={() => setEditMember(null)} className="p-1.5 hover:bg-secondary/5 rounded-lg transition-colors"><X size={18} className="text-secondary/90" /></button>
              </div>
              <div className="p-5 space-y-5" dir={language === 'ar' ? 'rtl' : 'ltr'}>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 sm:gap-4">
                  <div>
                    <label className="flex items-center gap-1.5 mb-2">
                      <span className="w-5 h-5 rounded-md bg-secondary/[0.06] flex items-center justify-center shrink-0"><Users size={11} className="text-secondary/90" /></span>
                ...
                <div>
                        ...
                        <button
                          ...
                          className={`text-xs px-2.5 py-1.5 rounded-lg font-dubai font-bold border transition-all ${
                            isSelected
                              ...
                              : 'bg-white text-secondary/90 border-secondary/[0.08] hover:border-secondary/20'
                          }`}
                        >
                          {ROLE_LABELS[r]}
                        </button>
                  ...
                  <p className="text-[10px] text-secondary/90 font-dubai mt-1.5">
