...
const Sidebar: React.FC<SidebarProps> = ({ isMobileOpen, onClose, onOpen }) => {
  ...
  const menuItems = [
    ...
    { icon: FileText, label: 'الطلبات', href: '/admin/requests' },
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        className="fixed right-0 top-0 h-full w-80 max-w-[85vw] bg-white shadow-[0_15px_60px_rgba(237,191,140,0.7)] border-l-2 border-primary/20 flex flex-col overflow-hidden z-50 lg:right-8 lg:top-8 lg:h-[calc(100vh-4rem)] lg:w-72 lg:rounded-2xl lg:border-2"
      >
      {/* معلومات المستخدم */}
      {session?.user && (
        <div className="p-6 border-b border-primary/20 bg-gradient-to-br from-accent/30 to-transparent">
          {/* زر الإغلاق للموبايل */}
          {isMobile && (
            <button
              onClick={onClose}
                ...
                <Link href={item.href} onClick={handleLinkClick}>
                  <motion.div
                    ...
                    className={`
                      ...
                      ${
                        isActive
                          ? 'bg-gradient-to-l from-primary to-primary/80 text-white shadow-[0_4px_15px_rgba(237,191,140,0.4)]'
      ...
      {/* زر تسجيل الخروج */}
      <div className="p-4 border-t border-primary/20 bg-gradient-to-br from-primary/5 to-transparent">
        <motion.button
          whileHover={{ x: -5 }}
          whileTap={{ scale: 0.98 }}
          ...
          className="
            ...
            text-red-600/80 hover:bg-red-500/10 hover:text-red-600 hover:shadow-sm
