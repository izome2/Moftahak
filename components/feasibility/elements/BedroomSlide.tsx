...
import { bedroomIcons } from '@/lib/feasibility/icons';
import { 
  bedroomItems, 
  bedroomCategories, 
  type BedroomItemDefinition 
} from '@/lib/feasibility/bedroom-items';
...
type BedroomCategory = keyof typeof bedroomCategories;
...
const ItemWidget: React.FC<ItemWidgetProps> = ({
  ...
  // للعناصر المخصصة، نستخدم الأيقونة المحفوظة، للعادية نستخدم bedroomIcons
  const IconComponent = isCustomItem 
    ? getCustomIcon(item.icon) 
    : (bedroomIcons[itemKey] || Package);
...
interface LibraryPopupProps {
  isOpen: boolean;
  onClose: () => void;
  onAddItem: (item: BedroomItemDefinition) => void;
  customItems: BedroomItemDefinition[];
...
const DragOverlayItem: React.FC<{ item: BedroomItemDefinition }> = ({ item }) => {
  const IconComponent = bedroomIcons[item.id] || item.icon || Package;
...
const LibraryPopup: React.FC<LibraryPopupProps> = ({ isOpen, onClose, onAddItem, customItems, onAddCustomItem, onDeleteCustomItem, getCustomPrice }) => {
  ...
  const [selectedCategory, setSelectedCategory] = useState<BedroomCategory | 'all' | 'custom'>('all');
  const [activeItem, setActiveItem] = useState<BedroomItemDefinition | null>(null);
  ...
  const handleDragStart = useCallback((event: DragStartEvent) => {
    const { active } = event;
    const item = active.data.current?.item as BedroomItemDefinition;
    if (item) {
      setActiveItem(item);
    }
  }, []);
  ...
  const filteredItems = useMemo(() => {
    ...
    return bedroomItems.filter((item) => {
      ...
      >
        ...
        <div className="bg-linear-to-r from-primary to-primary/80 p-4 flex items-center justify-between relative overflow-hidden">
          <div className="absolute inset-0 opacity-10">
            <Bed className="absolute -top-10 -right-10 w-40 h-40 text-secondary" />
          </div>
          ...
          <div className="flex items-center gap-3 relative z-10">
            ...
            <div>
              <h3 className="font-dubai font-bold text-secondary text-base">مكتبة عناصر غرفة النوم</h3>
        ...
        <div className="p-3 border-b border-secondary/10 space-y-3 bg-accent/20">
          ...
          <div className="flex gap-1.5 flex-wrap">
            ...
            {(Object.keys(bedroomCategories) as BedroomCategory[]).map((cat) => (
              <motion.button
                key={cat}
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-4 bg-accent/10" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          ...
          {selectedCategory !== 'custom' && Object.entries(groupedItems).map(([category, items]) => (
            <div key={category}>
              <h4 className="font-dubai font-bold text-secondary text-sm mb-3 flex items-center gap-2">
                <span className="w-1 h-5 bg-primary rounded-full" />
                {bedroomCategories[category as BedroomCategory]?.name || category}
...
const loadCustomItems = (): BedroomItemDefinition[] => {
  if (typeof window === 'undefined') return [];
  try {
    const saved = localStorage.getItem(CUSTOM_BEDROOM_ITEMS_KEY);
    if (saved) {
      const items = JSON.parse(saved);
      // تحويل العناصر المخصصة إلى BedroomItemDefinition
      return items.map((item: CustomItemData) => ({
        id: item.id,
        name: item.name,
        icon: item.icon,
        category: 'custom' as BedroomCategory,
...
const saveCustomItems = (items: BedroomItemDefinition[]) => {
  if (typeof window === 'undefined') return;
  try {
    localStorage.setItem(CUSTOM_BEDROOM_ITEMS_KEY, JSON.stringify(items));
  } catch (e) {
    console.error('Error saving custom items:', e);
  }
};

const BedroomSlide: React.FC<BedroomSlideProps> = ({
  data,
  isEditing = false,
  onUpdate,
  roomNumber = 1,
  ...
  const handleAddCustomItem = useCallback((newItem: CustomItemData) => {
    const bedroomItem = {
      id: newItem.id,
      name: newItem.name,
      icon: newItem.icon as unknown as typeof bedroomItems[0]['icon'],
      category: 'custom' as BedroomCategory,
      defaultPrice: newItem.defaultPrice,
    } as BedroomItemDefinition;
    const updatedItems = [...customItems, bedroomItem];
  ...
  const { setNodeRef, isOver } = useDroppable({
    id: `bedroom-${roomNumber}`,
    data: { type: 'bedroom', roomNumber },
  });
  ...
  const updateParent = (newItems: RoomItem[]) => {
    ...
    if (onUpdate) {
      const updatedRoom = {
        id: room?.id || 'bedroom-1',
        type: 'bedroom' as const,
        name: room?.name || 'غرفة النوم',
        number: roomNumber,
        items: newItems,
        totalCost,
      };
  ...
  const handleAddItem = (itemDef: BedroomItemDefinition) => {
    ...
    if (existingItem) {
      ...
      const newItem: RoomItem = {
        ...
        icon: isCustomItem ? ((itemDef.icon as unknown as string) || 'package') : itemDef.id,
        price: customPrice,
        quantity: 1,
  ...
  const handlePriceChange = (itemId: string, newUnitPrice: number) => {
    ...
    updateItemPrice(originalItemId, 'bedroom', newUnitPrice);
  };
  ...
  const handleImageChange = (itemId: string, image: string | undefined) => {
    ...
    const originalItemId = itemId.split('-').slice(0, -1).join('-') || itemId.split('-')[0];
    updateItemImage(originalItemId, 'bedroom', image || null);
  };
  ...
  const handleDescriptionChange = (itemId: string, description: string) => {
    ...
    const originalItemId = itemId.split('-').slice(0, -1).join('-') || itemId.split('-')[0];
    updateItemDescription(originalItemId, 'bedroom', description || null);
  };
      ...
      >
        ...
        >
          <div className="absolute -top-8 -left-8 opacity-[0.08] pointer-events-none">
            <Bed className="w-32 sm:w-44 lg:w-56 h-32 sm:h-44 lg:h-56 text-primary" strokeWidth={1.5} />
          </div>
          ...
          <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div className="flex items-center gap-2.5 sm:gap-4">
              ...
              >
                <Bed className="w-6 h-6 sm:w-8 sm:h-8 lg:w-10 lg:h-10 text-primary" strokeWidth={2} />
              </motion.div>
              <div>
                <EditableSectionTitle
                  title={roomNumber > 1 ? `غرفة النوم ${roomNumber}` : 'غرفة النوم'}
                  subtitle="أضف وعدّل عناصر ومستلزمات غرفة النوم"
                  isEditing={isEditing}
                />
              </div>
            </div>
          ...
          {items.length === 0 ? (
            ...
            >
              ...
              <p className="font-dubai text-secondary/50 text-sm max-w-sm mb-6">
                ابدأ بإضافة عناصر غرفة النوم من المكتبة لحساب التكلفة المتوقعة
              </p>
          ...
          >
            ...
            <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <h4 className="font-dubai font-bold text-xl">إجمالي تكلفة غرفة النوم</h4>
