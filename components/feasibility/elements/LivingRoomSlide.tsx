...
/**
 * LivingRoomSlide - شريحة غرفة المعيشة (الصالة)
 * 
 * تصميم احترافي مع:
...
import { livingRoomIcons } from '@/lib/feasibility/icons';
import { 
  livingRoomItems, 
...
interface LivingRoomSlideProps {
  data: RoomSlideData;
  isEditing?: boolean;
...
type LivingRoomCategory = keyof typeof livingRoomCategories;
      ...
      <div className="flex flex-col items-center gap-2 relative z-10">
        ...
        <div className="text-center">
          ...
          <span className="text-xs text-primary font-dubai font-bold">
            {formatPrice(displayPrice ?? item.defaultPrice)} {currencySymbol}
          </span>
        </div>
      </div>
    </motion.div>
...
const DragOverlayItem: React.FC<{ item: LivingRoomItemDefinition }> = ({ item }) => {
...
const LibraryPopup: React.FC<LibraryPopupProps> = ({ isOpen, onClose, onAddItem, customItems, onAddCustomItem, onDeleteCustomItem, getCustomPrice }) => {
  ...
  const handleDragStart = useCallback((event: DragStartEvent) => {
    const { active } = event;
    const item = active.data.current?.item as LivingRoomItemDefinition;
    if (item) {
      setActiveItem(item);
    }
  }, []);
      ...
      >
        ...
        <div className="bg-linear-to-r from-primary to-primary/80 p-4 flex items-center justify-between relative overflow-hidden">
          <div className="absolute inset-0 opacity-10">
            <Sofa className="absolute -top-10 -right-10 w-40 h-40 text-secondary" />
          </div>
          
          <div className="flex items-center gap-3 relative z-10">
            <div className="w-10 h-10 bg-white/20 rounded-xl flex items-center justify-center backdrop-blur-sm">
              <Sofa className="w-5 h-5 text-secondary" />
            </div>
            <div>
              <h3 className="font-dubai font-bold text-secondary text-base">مكتبة عناصر الصالة</h3>
        ...
        <div className="p-3 border-b border-secondary/10 space-y-3 bg-accent/20">
          ...
          <div className="flex gap-1.5 flex-wrap">
            ...
            {(Object.keys(livingRoomCategories) as LivingRoomCategory[]).map((cat) => (
              <motion.button
                key={cat}
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-4 bg-accent/10" style={{ scrollbarWidth: 'none', msOverflowStyle: 'none' }}>
          ...
          {selectedCategory !== 'custom' && Object.entries(groupedItems).map(([category, items]) => (
            <div key={category}>
              <h4 className="font-dubai font-bold text-secondary text-sm mb-3 flex items-center gap-2">
                <span className="w-1 h-5 bg-primary rounded-full" />
                {livingRoomCategories[category as LivingRoomCategory]?.name || category}
...
const LivingRoomSlide: React.FC<LivingRoomSlideProps> = ({
  ...
  const { setNodeRef, isOver } = useDroppable({
    ...
    data: { type: 'livingRoom', roomNumber },
  });
  ...
  const updateParent = (newItems: RoomItem[]) => {
    ...
    if (onUpdate) {
      const updatedRoom = {
        id: room?.id || 'livingRoom-1',
        type: 'living-room' as const,
        name: room?.name || 'الصالة',
        number: roomNumber,
        items: newItems,
        totalCost,
      };
  ...
  const handleDescriptionChange = (itemId: string, description: string) => {
    ...
    const originalItemId = itemId.split('-').slice(0, -1).join('-') || itemId.split('-')[0];
    updateItemDescription(originalItemId, 'living-room', description || null);
  };
      ...
      >
        ...
        >
          <div className="absolute -top-8 -left-8 opacity-[0.08] pointer-events-none">
            <Sofa className="w-32 sm:w-44 lg:w-56 h-32 sm:h-44 lg:h-56 text-primary" strokeWidth={1.5} />
          </div>
          ...
          <div className="relative z-10 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3 sm:gap-4">
            <div className="flex items-center gap-2.5 sm:gap-4">
              ...
              >
                <Sofa className="w-6 h-6 sm:w-8 sm:h-8 lg:w-10 lg:h-10 text-primary" strokeWidth={2} />
              </motion.div>
              <div>
            ...
            >
              ...
              >
                ...
                <div 
                  ...
                  style={{ boxShadow: SHADOWS.icon }}
                >
                  <Sofa className="w-16 h-16 text-primary" strokeWidth={1.5} />
                </div>
              </motion.div>
              ...
              <p className="font-dubai text-secondary/50 text-sm max-w-sm mb-6">
                ابدأ بإضافة عناصر الصالة من المكتبة لحساب التكلفة المتوقعة
              </p>
        ...
        {items.length > 0 && (
          ...
          >
            <div className="absolute -top-10 -left-10 opacity-10 pointer-events-none">
              <Sofa className="w-40 h-40 text-white" strokeWidth={1.5} />
            </div>

            <div className="relative z-10 flex flex-col sm:flex-row items-center justify-between gap-4">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 bg-white/10 rounded-2xl flex items-center justify-center backdrop-blur-sm">
                  <Sofa className="w-7 h-7 text-primary" />
                </div>
                <div>
                  <h4 className="font-dubai font-bold text-xl">إجمالي تكلفة الصالة</h4>
...
export default LivingRoomSlide;
