  ...
  return (
    <div className="flex-1 flex flex-col gap-3" dir={language === 'ar' ? 'rtl' : 'ltr'}>
          ...
          <div className="flex items-center gap-2 sm:gap-3.5 min-w-0">
            ...
            <div className="min-w-0">
              <h1 className="text-base sm:text-xl md:text-2xl font-bold text-secondary font-dubai tracking-tight truncate">
                ...
                {data && (
                  <span className="text-sm font-normal text-secondary/90 mr-2">
                    — {data.investor.firstName} {data.investor.lastName}
                  </span>
                )}
              </h1>
              {data?.investor.email && (
                <p className="text-xs text-secondary/90 font-dubai mt-0.5 hidden sm:flex items-center gap-1">
      ...
      <div className="bg-white/95 rounded-2xl shadow-sm border border-secondary/[0.08] flex-1 p-3 sm:p-4 md:p-6 lg:p-8 space-y-4 sm:space-y-5 overflow-y-auto">
        {/* Loading State */}
        {isLoading && (
          <div className="py-16 flex flex-col items-center gap-3 text-secondary/90">
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {data.investor.email && (
                    <p className="flex items-center gap-1 text-sm text-white/90 font-dubai mt-0.5">
                ...
                <div className="text-center shrink-0 bg-white/5 border border-white/8 rounded-xl px-4 py-3">
                  <p className="text-[11px] text-white/90 font-dubai">{t.accounting.investorDetail.investmentsCount}</p>
          ...
          {data.investments.length === 0 ? (
            ...
            >
              <div className="w-12 h-12 rounded-2xl bg-secondary/[0.03] mx-auto mb-3 flex items-center justify-center">
                <Building2 className="w-6 h-6 text-secondary/80" />
              </div>
              <p className="text-sm text-secondary/90 font-dubai">{t.accounting.investorDetail.noInvestments}</p>
            </motion.div>
          ) : (
            <div className="space-y-4">
              <motion.div 
                ...
                className="flex items-center gap-2 text-xs text-secondary/90 font-dubai"
              >
